...
The Android Actions workflow now supports a stable signed release APK without storing signing material in Git:

- Release signing is configured in `app/build.gradle.kts` from environment variables supplied by GitHub Actions.
- The workflow reconstructs the keystore only in the ephemeral runner from `IMUX_KEYSTORE_BASE64`.
- The keystore is deleted in an `always()` cleanup step after the build.
- `:app:assembleRelease` is used instead of only `assembleDebug`.
- `apksigner verify --verbose` verifies the generated APK before upload.
- The APK is uploaded with `actions/upload-artifact@v4` as `Imux-Launcher-release.apk` with a 30-day retention period.

Required GitHub Actions Secrets:

- `IMUX_KEYSTORE_BASE64`
- `IMUX_KEYSTORE_PASSWORD`
- `IMUX_KEY_ALIAS`
- `IMUX_KEY_PASSWORD`

The signing key must remain stable across future builds. Replacing it with a different keystore/key will prevent Android from accepting the APK as an update over an installation signed with the previous key.
...
The project continues to use compile/target SDK 35, JDK 17, Gradle 8.11.1, NDK 27.0.12077973 and CMake 3.31.6. CI now performs the release path: Gradle configuration -> Android native/Kotlin compilation -> signed release APK -> signature verification -> Actions artifact upload.

The repository does not contain a keystore, password, alias or other signing secret.
...
The release workflow must be considered validated only after a run has successfully completed `assembleRelease`, `apksigner verify`, and artifact upload. The GitHub Actions run generated by the signing workflow will provide that final validation result.
...
Validate the signed release artifact end-to-end. After the UI foundation is verified, introduce a small Game Launch Boundary and then continue the renderer milestone with a genuine Android surface/native-window path and one real GPU backend. Do not enable Play until a real launch path exists.