...
@Composable private fun StartupGate(settings: LauncherSettings, content: @Composable () -> Unit) {
    ...
    val tasks = remember {
        listOf(
            DelayedStartupTask("configuration", "Checking configuration", 100),
            DelayedStartupTask("updates", "Checking updates", 140),
            DelayedStartupTask("resources", "Loading resources", 120),
            DelayedStartupTask("runtime", "Preparing runtime", 140),
            DelayedStartupTask("renderer", "Initializing renderer", 100)
        )
    }
    LaunchedEffect(Unit) {
    ...
    if (ready && settings.animations.profile == AnimationProfile.OFF) {
        ...
        AnimatedContent(
            targetState = ready,
            transitionSpec = { if (settings.animations.profile == AnimationProfile.REDUCED) tween(90) else tween(180) },
            label = "startup"
...
@Composable private fun LoadingScreen(settings: LauncherSettings, status: String, progress: Float) {
    ...
    Surface(Modifier.fillMaxSize()) {
        Column(Modifier.fillMaxSize().padding(40.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
            ...
            LinearProgressIndicator(progress = { animatedProgress }, modifier = Modifier.fillMaxWidth(.55f))
            Spacer(Modifier.height(16.dp))
            Text(status, style = MaterialTheme.typography.titleMedium)
            Text("${(animatedProgress * 100).toInt()}%", style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
    }
}
...
@Composable private fun LauncherShell(versions: List<VersionInfo>?, settings: LauncherSettings, vm: LauncherSettingsViewModel) {
    ...
    val widthClass = calculateWindowSizeClass(context as Activity).widthSizeClass
    ...
    Row(Modifier.fillMaxSize().padding(20.dp), horizontalArrangement = Arrangement.spacedBy(18.dp)) {
        Column(Modifier.weight(1.2f).fillMaxHeight(), verticalArrangement = Arrangement.spacedBy(14.dp)) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                Text("Imux", style = MaterialTheme.typography.headlineLarge, fontWeight = FontWeight.Bold, modifier = Modifier.weight(1f))
            ...
            Card(Modifier.fillMaxWidth().animateContentSize()) {
                Column(Modifier.padding(24.dp)) {
                    Text("Selected build", style = MaterialTheme.typography.labelLarge)
                    Text(selected?.name ?: "No build available", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.SemiBold)
                    Text(selected?.repository ?: "Checking catalog…", color = MaterialTheme.colorScheme.onSurfaceVariant)
                    Spacer(Modifier.height(20.dp))
                    Button(onClick = { launching = true }, enabled = selected != null && !launching, modifier = Modifier.fillMaxWidth()) {
                        Icon(Icons.Default.PlayArrow, null); Spacer(Modifier.width(8.dp)); Text(if (launching) "Preparing…" else "Launch")
                    }
            ...
            Text("RAM ${settings.performance.memoryMb} MB · ${settings.game.fpsLimit} FPS", style = MaterialTheme.typography.labelLarge)
        }
                    ...
                    LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        items(versions, key = { it.id }) { version ->
                            ListItem(
                                ...
                                supportingContent = { Text(version.repository) },
                                leadingContent = { Icon(Icons.Default.Gamepad, null) },
                                modifier = Modifier.fillMaxWidth(),
                                tonalElevation = if (version.id == selected?.id) 2.dp else 0.dp
                            )
                        }
                    }
                }
